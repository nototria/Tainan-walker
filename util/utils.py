...
def read_path_from_csv(csv_file_path):
    """
    Reads the CSV file generated by find_k_path and reconstructs the path
    as a list of dicts with lat/lng coordinates.

    Assumes CSV columns:
    1,start_x,start_y,end_x,end_y,distance,green_ratio,shade,pavement_ratio

    Returns:
        path: list of {"lat": float, "lng": float} ordered along the path
    """
    path = []
    ...
    with open(csv_file_path, newline='', encoding='utf-8') as csvfile:
        ...
        # We assume edges are listed in path order
        # So start_x, start_y of first edge is path start
        # Then append end_x, end_y of each edge

        prev_end = None
        for row in reader:
            # Parse floats, ignoring the first column (constant 1)
            _, start_x, start_y, end_x, end_y, *_ = row
            start_x, start_y = float(start_x), float(start_y)
            end_x, end_y = float(end_x), float(end_y)

            if not path:
                # Add the starting point first
                path.append({"lat": start_y, "lng": start_x})

            # Append the end point of current edge
            # Check if it's the same as previous end point to avoid duplicates
            if prev_end != (end_x, end_y):
                path.append({"lat": end_y, "lng": end_x})
                prev_end = (end_x, end_y)
